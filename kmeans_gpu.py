...
def initialize(X, num_clusters):
    ...
    num_samples = len(X)
    ### <--- [MODIFIED] ---------------------------------------
    # Use torch RNG on the same device to reduce cross-library randomness drift.
    indices = torch.randperm(num_samples, device=X.device)[:num_clusters]
...
def kmeans(
    ...
    while True:
        ...
        for index in range(num_clusters):
            ...
            # Keep previous center if a cluster is empty to avoid NaNs.
            if selected.numel() == 0:
                continue
